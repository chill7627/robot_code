...
class PIController:
    def __init__(self, proportional_constant=0, integral_constant=0, windup_limit=None):
        ...
        self.windup_limit = windup_limit # for keeping integral constant from becoming too large
        # Running sums
        self.integral_sum = 0
    ...
    def handle_integral(self, error):
        if self.windup_limit is None or (abs(self.integral_sum) < self.windup_limit) \
